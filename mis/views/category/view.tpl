...
<{block name="content"}>
    ...
    <{if isset($action)}>
    <form method="post" action="<{if $action eq 2}><{'/category/edit'}><{elseif $action eq 1}><{'/category/add'}><{/if}>" id="f1" class="user_form"><{/if}>
    ...
    <table cellpadding="0" cellspacing="0" class="tb-form top10" style="padding: 30px;width:900px;">
        ...
        <tr>
            <td class="td1"><{if isset($action)}><em class="required">*</em><{/if}>类别名：</td>
        ...
        <{if !isset($action)}>
            <tr>
                ...
                <td style="padding-top:15px;padding-bottom: 15px">
                    <{$info.status|default:0|replace:0:'未删除'|replace:1:'已删除'}>
                </td>
            </tr>
        <{/if}>
