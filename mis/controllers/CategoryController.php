...
class CategoryController extends MisBaseController{
    /**
     * 获取司机列表
     * */
...
//        echo json_encode($arrTplData);exit;
    ...
    public function actionAdd(){
        ...
        if(!empty($query)){
            ...
            $objProduct->is_del = ApiConfig::IS_DEL_NOT;
            $ret = $objProduct->save();
            if($ret){
                $this->redirect('/category/list');
            }else{
                echo "<script>alert('添加失败');window.location.href='/category/add';</script>";
            }
        }else{
            $data['action'] = 1;
    ...
    public function actionEdit(){
        ...
        try{
            ...
            $objProduct = Category::findOne(['id' => $id]);
            if(!$objProduct){
            ...
            if(!empty($query)){
                ...
                Util::pageNotFund('修改成功',"/category/list");
            }
            $data['action'] = 2;
            $info = Category::find()
                ->where(['id' => $id])
