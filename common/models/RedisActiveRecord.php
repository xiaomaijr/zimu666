...
//        echo $query->createCommand()->getRawSql();exit;
    ...
    {
        $idInfo = static::getDataByConditions($condition, 'id desc', 0, 0, 'id');
        if(!$idInfo){
            return 0;
        }
        $ids = ApiUtils::getCols($idInfo, 'id');
        $ret = parent::updateAll($attributes, ['id' => $ids], $params); // TODO: Change the autogenerated stub
        if(!$ret){
            return $ret;
        }
        $cache = self::getCache();
