...
//        'expire'    =>  1800,            // 验证码过期时间（s）
        ...
        'path'      =>  __DIR__ .  '/../../api/web/static/', //验证码图片存放路径
    );
    ...
    public function check($code, $id = '') {
        $key = CacheKey::getCacheKey($id, $this->seKey);
//        $key = $this->authcode($this->seKey).$id;
        $cache = new Cache();
        // 验证码不能为空
        $tempKey = $cache->get($key['key_name']);
        $secode = json_decode($tempKey, true);

        if(empty($code) || empty($secode)) {
            return false;
        }
        // redis 过期
        if(time() - $secode['verify_time'] > $key['expire']) {
            $cache->set($key['key_name'],null);
            return false;
        }

        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && ($cache->set($key['key_name'],null));
            return true;
        }
        return false;
    }
...
//        $key        =   $this->authcode($this->seKey);
        ...
        $secode['verify_code'] = $code; // 把校验码保存到reids
        $secode['verify_time'] = time();  // 验证码创建时间
        $cache = new Cache();
        $cache->set($key['key_name'], json_encode($secode));
