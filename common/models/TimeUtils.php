...
class TimeUtils
{
     ...
     *              返回精度，支持ms和s精度，默认为ms
     */
    public function __construct($precision = Timer::PRECISION_MS)
    {
     ...
     *              false - 重置所有定时器
     */
    public function reset($completely = false)
    {
        if($completely)
        {
            $this->arrTimer = array();
     ...
     *         false - 指定定时器不存在
     */
    public function getTotalTime($strName = null, $precision = null)
    {
        if($strName !== null)
        {
            if(!array_key_exists($strName, $this->arrTimer))
